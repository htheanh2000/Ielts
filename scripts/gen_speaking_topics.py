...
// Convert recorded webm/opus blob to WAV PCM 16-bit mono 16kHz (Azure requirement)
...
async function uploadAndScore(blob, refText) {
  ...
  resultEl.innerHTML = `<div class="score-loading">Đang chuẩn hoá audio…</div>`;

  let wavBlob;
  try {
    wavBlob = await blobToWav16k(blob);
  } catch (e) {
    resultEl.innerHTML = `<div class="score-error">Không decode được audio: ${e.message}</div>`;
    return;
  }
  ...
  try {
    ...
    const data = await r.json();
    if (!r.ok || data.error) {
      resultEl.innerHTML = `<div class="score-error">Lỗi: ${data.error || 'HTTP ' + r.status}${data.detail ? ' — ' + data.detail : ''}</div>`;
      return;
    }
...
// Map Azure 0-100 PronScore to IELTS band (rough heuristic — Azure does not output band directly)
function pronScoreToBand(s) {
  if (s >= 90) return 8.5;
  if (s >= 85) return 8.0;
  if (s >= 80) return 7.5;
  if (s >= 75) return 7.0;
...
function renderScore(container, data, blob) {
  ...
  // { RecognitionStatus, DisplayText, NBest: [{ Display, PronunciationAssessment, Words: [...] }] }
  const top = (data.NBest && data.NBest[0]) || {};
  const pa = top.PronunciationAssessment || {};
  const words = top.Words || [];

  const pron = pa.PronScore != null ? pa.PronScore : (pa.PronunciationScore || 0);
  const accuracy = pa.AccuracyScore;
  const fluency = pa.FluencyScore;
  const completeness = pa.CompletenessScore;
  const band = pronScoreToBand(pron);

  let wordsHtml = '';
  for (const w of words) {
    const wpa = w.PronunciationAssessment || {};
    const acc = wpa.AccuracyScore != null ? wpa.AccuracyScore : 0;
    const errType = wpa.ErrorType || 'None';
    let cls = acc >= 80 ? 'good' : acc >= 60 ? 'ok' : 'poor';
    if (errType === 'Omission' || errType === 'Insertion') cls = 'poor';
    const tip = `${Math.round(acc)}/100${errType !== 'None' ? ' · ' + errType : ''}`;
    wordsHtml += `<span class="score-word ${cls}" title="${tip}">${w.Word}</span> `;
  }

  let subRow = '';
  if (accuracy != null) subRow += `<div class="sub">Accuracy<b>${Math.round(accuracy)}</b></div>`;
  if (fluency != null)  subRow += `<div class="sub">Fluency<b>${Math.round(fluency)}</b></div>`;
  if (completeness != null) subRow += `<div class="sub">Completeness<b>${Math.round(completeness)}</b></div>`;
  if (pron != null) subRow += `<div class="sub">Overall<b>${Math.round(pron)}/100</b></div>`;
  ...
  container.innerHTML = `
    <div class="score-card">
      <div class="band-row">
        ...
        <div class="band-label">Pronunciation band <span style="opacity:0.6">(estimate from Azure PronScore)</span></div>
      </div>
      ${subRow ? `<div class="sub-row">${subRow}</div>` : ''}
      <div class="words-label">Word-level accuracy</div>
